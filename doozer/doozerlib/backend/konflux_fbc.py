...
class KonfluxFbcRebaser:
    ...
    async def rebase(self, metadata: ImageMetadata, bundle_build: KonfluxBundleBuildRecord, fbc_version: str, fbc_release: str):
        ...
        # Update the FBC repo
        await self._rebase_dir(build_repo, bundle_build, logger)
        ...
        # await build_repo.tag(f"{fbc_version}-{fbc_release}")
        if self.push:
            await build_repo.push()

        logger.info("rebase complete")

    async def _rebase_dir(self, build_repo: BuildRepo, bundle_build: KonfluxBundleBuildRecord, logger: logging.Logger):
        ...
        if related_images:
            ...
            if target_entry:
                new_pullspec = f"{constants.DELIVERY_IMAGE_REGISTRY}/openshift4/{bundle_build.name}@sha256:{bundle_build.image_tag}"
