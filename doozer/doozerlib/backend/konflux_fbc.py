...
class KonfluxFbcImporter:
    ...
    async def _update_dir(self, metadata: ImageMetadata, build_repo: BuildRepo, index_image: str, logger: logging.Logger):
        ...
        org_catalog_file = org_catalog_dir.joinpath("catalog.yaml")
        logger.info("Writing original catalog blobs to %s", org_catalog_file)
        ...
        await opm.generate_basic_template(org_catalog_file, template_file)
    ...
    async def _get_catalog_blobs_from_index_image(self, index_image: str, package_name):
        ...
        filtered_blobs = self.filter_catalog_blobs(blobs, {package_name})
...
class KonfluxFbcBuilder:
    ...
    async def _start_build(
        ...
        app_name = "fbc-" + self.group.replace("openshift-", "ocp-").replace(".", "-")
